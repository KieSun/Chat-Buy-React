...
const MyOrder = asyncComponent(() => import("./myOrder.jsx"));
...
let DashBoard = class DashBoard extends React.Component {
    ...
    render() {
        ...
        return (<div>
        {currentNavBar ? (<NavBar className="nav">{currentNavBar.title}</NavBar>) : null}
        <Switch>
          
          {list.map(v => (<Route exact key={v.path} path={v.path} component={v.component}/>))}
        ...
        <div className="dashBoard-wrapper">
          <TabBar hidden={!currentNavBar}>
            ...
            {list.filter(v => v.type !== type).map(v => (<TabBar.Item icon={{ uri: require(`../images/${v.imgName}.png`) }} selectedIcon={{
            uri: require(`../images/${v.imgName}-sel.png`)
        }} badge={v.path === "/messageList" ? 1 : 0 && noReadCount} title={v.title} key={v.title} selected={location.pathname === v.path} onPress={() => {
            history.push(v.path);
        }}/>))}
          </TabBar>
        </div>
