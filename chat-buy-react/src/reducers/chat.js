...
export default function(state = initialState, action) {
  switch (action.type) {
    ...
    case GET_MESSAGE_LIST:
      return state.merge({messageList: sortMessageList(action.payload), userId: action.userId, noReadCounts: action.noReadCounts, noReadCount: action.noReadCounts.reduce((sum, value) => sum + value, 0)})
    case SET_CURRENT_LIST:
      return state.merge({currentChatList: action.payload ? action.payload.get('messages') : List([]), currentMessageId: action.messageId})
    case CLEAN_NO_READ:
      return state.merge({
        noReadCounts: action.noReadCounts,
